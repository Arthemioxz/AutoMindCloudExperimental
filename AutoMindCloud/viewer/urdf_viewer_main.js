...
// Viewer moderno + thumbnails del script estable + IA opt-in.
//
// - Thumbnails UI:
//     * Se generan con renderer offscreen.
//     * Se usan DIRECTO en el panel (sin cambiar tamaño).
// - IA_Widgets (opt-in):
//     * Genera thumbnails por componente.
//     * Además genera UNA vista Iso del robot completo como contexto.
//     * Comprime SOLO las imágenes enviadas a IA (~5KB c/u aprox).
//     * Envía TODO en una sola llamada batch al callback de Colab.
//     * Recibe { assetKey: descripcion } y actualiza app.componentDescriptions.
//     * Lanza evento 'ia_descriptions_ready' para que ComponentsPanel refresque.
...
export function render(opts = {}) {
  ...
  // 9) IA opt-in
  if (IA_Widgets) {
    debugLog('[IA] IA_Widgets=true → bootstrap IA (batch + iso robot)');
...
function buildOffscreenForThumbnails(core) {
  ...
  // Igualar configuración del renderer principal
  if (core.renderer) {
    ...
    if ('outputColorSpace' in renderer && 'outputColorSpace' in core.renderer) {
      ...
      'outputEncoding' in core.renderer
    ) {
  ...
  function snapshotRobotIso() {
    ...
    if (box.isEmpty()) {
      debugLog('[Thumbs] snapshotRobotIso box vacío');
      return null;
    }
    ...
    renderer.render(scene, camera);
    const url = canvas.toDataURL('image/png');
    debugLog('[Thumbs] snapshotRobotIso generado');
    return url;
  }
  ...
  return {
    ...
    async robotIso() {
      try {
        ...
        debugLog('[Thumbs] Error robotIso', String(e));
        return null;
      }
    },
    destroy() {
...
/* ================= IA opt-in: batch describe_component_images ================= */
...
function bootstrapComponentDescriptions(app, assetToMeshes, off) {
  ...
  (async () => {
    try {
      ...
      // Thumbnails por componente → comprimidos a ~5KB SOLO para IA
      ...
      // Iso del robot como contexto adicional
      let robotIsoB64 = null;
      try {
        if (off.robotIso) {
          const isoUrl = await off.robotIso();
          if (isoUrl) {
            // un poco más grande para contexto; puedes bajar a 5 si quieres
            robotIsoB64 = await makeApproxSizedBase64(isoUrl, 10);
          }
        }
      } catch (e) {
        debugLog('[IA] Error generando robotIso', String(e));
      }

      const payload = {
        entries,
        robot_iso: robotIsoB64 ? { image_b64: robotIsoB64 } : null,
      };
...
async function makeApproxSizedBase64(dataURL, targetKB = 5) {
  try {
    ...
    // ratio según tamaño actual vs objetivo
