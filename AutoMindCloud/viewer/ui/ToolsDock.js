...
// --- Injected from .py (behavior kept identical): 'h' dock toggle + button overrides + MutationObserver ---
export function wireDockHotkeysAndViewOverrides(app){
  function handleKey(e){
    const k = (e.key || '').toLowerCase();
    if(k === 'i'){ e.preventDefault(); app.isolateSelectedComponent?.(); }
    if(k === 'h'){
      e.preventDefault();
  ...
  document.addEventListener('click', (ev)=>{
    ...
    app.navigateToViewFixedDistance?.(label, 750);
    ev.preventDefault();
    ev.stopPropagation();
  }, true);
}
