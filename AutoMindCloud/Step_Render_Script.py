import sympy  # (kept as in your snippet)
import gdown
import cascadio
import trimesh
import base64
...
def _to_data_url_audio(file_path):
    """
    If an audio file exists, return data:audio/mpeg;base64,<...> (works with .mp3).
...
def Step_Render(Step_Name, target_size=2.0, click_sound_path=None, background=0xf6fafb):
    """
    STEP -> GLB -> scaled viewer (teal/white UI). Features:
      - Render modes: Solid / Wireframe / X-Ray / Ghost
      - Explode: (not applicable to single-GLB; omitted)
      ...
      - Click sound system with overlapping playback (single-fire per click)

    click_sound_path: local path to an .mp3 to embed as data URL (optional)
    background: THREE scene color (int or None)
    """
    ...
    if current_size <= 0:
        current_size = 1.0
    scale_factor = (float(target_size) / float(current_size)) if target_size else 1.0
    ...
    # Optional click sound as data URL
...
<head>
  ...
  <style>
    ...
    .panel {{
      background: #ffffff; border: 1px solid #d7e7e7; border-radius: 18px;
      box-shadow: 0 12px 36px rgba(0,0,0,0.14); pointer-events: auto; overflow: hidden;
    }}
    .btn {{
    ...
    .hdr {{ display:flex; align-items:center; justify-content:space-between; gap:8px;
            padding:10px 12px; border-bottom:1px solid #d7e7e7; background: rgba(20,184,185,0.12); }}
    select, input[type="range"] {{
      padding: 8px; border: 1px solid #d7e7e7; border-radius: 10px; accent-color: #0ea5a6;
    }}
    .tools-toggle {{
      position: absolute; right: 14px; top: 14px;
      padding: 8px 12px; border-radius: 12px; border: 1px solid #d7e7e7;
      background: #ffffff; color: #0b3b3c; font-weight: 700;
      box-shadow: 0 12px 36px rgba(0,0,0,0.14); pointer-events: auto; z-index: 10000;
    }}
    .components-btn {{
...
<body>
  ...
  (function() {{
    const THEME = {{
      teal: 0x0ea5a6,
      bgCanvas: {bg_js}, // null -> transparent
    }};

    // --- Audio (single-fire per click, overlapping allowed) ---
    ...
    dirLight.position.set(3,4,2); dirLight.castShadow = true;
    ...
    const grid = new THREE.GridHelper(10, 20, 0x84d4d4, 0xdef3f3); grid.visible = false; groundGroup.add(grid);
    ...
    loader.parse(arrayBuffer, '', function(gltf) {{
      ...
      model.traverse(n => {{
        if (n.isMesh && n.material) {{
          ...
          n.geometry && n.geometry.computeVertexNormals && n.geometry.computeVertexNormals();
        }}
      }});
      scene.add(model);
      centerAndFrame();
    ...
    function centerAndFrame(pad=1.08){{
      if (!model) return;
    ...
    // -------- Section plane (with visible teal sheet) --------
    ...
    function updateSectionPlane(distNorm){{
      ...
      const plane = new THREE.Plane(n, -dist);  // model already centered at origin
    ...
    function viewTop(){{
      if (!model) return;
      const box=new THREE.Box3().setFromObject(model); const s=box.getSize(new THREE.Vector3()); const d=Math.max(s.x,s.y,s.z)*1.9;
    ...
    const togGround = mkToggle('Ground & shadows', true);
    ...
    fitBtn.addEventListener('click', ()=>{{ playClick(); centerAndFrame(1.06); }});
    ...
    togGround.cb.addEventListener('change', ()=>{{ playClick(); ground.visible = !!togGround.cb.checked; dirLight.castShadow = !!togGround.cb.checked; }});
