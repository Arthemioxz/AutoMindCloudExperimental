...
<script type="module">
  ...
  async function latestShaFull(){
    try{
      ...
      if (!r.ok) throw new Error('GitHub API not ok: ' + r.status);
      const j = await r.json();
      if (!j.sha) throw new Error('No sha in response');
      return j.sha; // ✅ FULL 40-char SHA
    }catch(e){
      console.warn('[URDF] latestShaFull() failed, using branch fallback:', e);
      return null;
    }
  }
  ...
  // Import entry module from latest FULL SHA (fallback to branch)
  let mod = null;
  ...
  try {{
    ...
    mod = await import('https://cdn.jsdelivr.net/gh/' + repo + '@' + branch + '/' + compFile);
  }}
