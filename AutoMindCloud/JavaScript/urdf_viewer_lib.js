/* urdf_viewer_lib.js
   Exposes window.URDFViewer (no rendering here).
*/
(function (root) {
  'use strict';
  ...
  function collectMeshesInLink(linkObj){
    ...
    while (stack.length){
      ...
      for (let i=1;i<kids.length;i++) stack.push(kids[i]);
    }
    return t;
  }
  ...
  URDFViewer.render = function(opts){
    ...
    // single handler for GUI button presses (optional)
    ...
    async function buildGallery(listEl){
      ...
      for (const ent of entries){
        ...
        desc.textContent = (opts && opts.descriptions && opts.descriptions[ent.base]) || ' ';
    ...
    const hoverKeyFor = (meshHit)=>{
      ...
      if (selectMode==='link'){
        const link = api.linkSet ? (function findAncestorLink(o, linkSet){ while (o){ if (linkSet && linkSet.has(o)) return o; o=o.parent; } return null; })(meshHit, api.linkSet) : null;
    ...
    function processHover(e){
      ...
      const joint = (function findAncestorJoint(o){
        while (o){
          if (o.jointType && isMovable(o)) return o;
          if (o.userData && o.userData.__joint && isMovable(o.userData.__joint)) return o.userData.__joint;
          o = o.parent;
    ...
    function onPointerDown(e){
      ...
      if (joint && isMovable(joint)) (function startJointDrag(joint, ev){
        const originW = joint.getWorldPosition(new THREE.Vector3());
        const qWorld  = joint.getWorldQuaternion(new THREE.Quaternion());
        const axisW   = (joint.axis||new THREE.Vector3(1,0,0)).clone().normalize().applyQuaternion(qWorld).normalize();
        const dragPlane = new THREE.Plane().setFromNormalAndCoplanarPoint(axisW.clone(), originW);
    ...
    state.ui = (function createUIWrapper(){
      const { root, btn, panel, list, showAllBtn } = (function(){ return createUI(); })();
      (function attachUIInteractions(){
        ...
        showAllBtn.addEventListener('click', ()=>{ buttonClicked(); showAllAndFrame(); });
    ...
    (function loadAndAnimate(){
      if (opts && opts.urdfContent) (function loadURDFWrap(t){ return loadURDF(t); })(opts.urdfContent);
      animate();
    })();
